...
- **提交之前**: 必须运行 `pnpm format` 并修复所有问题，确保 0 错误、0 警告后方可提交。

## 分支与构建流程

- **开发分支**: 所有开发一律在 `dev` 分支进行，禁止直接改动 `main`。
- **合并由用户决定**: 是否合并到 `main`、何时合并，一律由用户主动提出，**严禁擅自执行合并到 `main`**。开发完成只需推送到 `dev`，合并需等待用户明确指示。
- **合并主线**: 当用户明确要求合并时，先合并到 `main`，再由 GitHub Actions 的 **Publish** 工作流自动构建 Docker 镜像。
- **合并后切回来源分支**: 代码合并到 `main` 后，必须立即切回合并前的来源分支（通常是 `dev`），禁止停留在 `main` 分支进行后续开发。
- **禁止擅自构建**: 每次要构建工作流，必须由用户主动提出。**不得自行触发构建。**
- **禁止本地构建镜像**: 严禁在本地构建 Docker 镜像，镜像一律由 GitHub Actions 的 **Publish** 工作流在云端构建。本地只能修改 Dockerfile，构建效果需通过远程工作流验证。
- **触发前询问**: 在任何触发构建前，必须先向用户确认，得到明确同意后才能执行。
- **版本号规则**: 用户要求构建时——
  - 若指定了版本号，直接使用该版本号；
  - 若未指定版本号，则版本号自动 +1（递增 patch）。
