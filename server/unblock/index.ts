...
import { getSources, getCustomSources, setCustomSources, getDefaultSources } from "../unm";
import { initUnblockProxy } from "./proxy";
...
export const initUnblockAPI = async (fastify: FastifyInstance) => {
  ...
  // 获取当前解灰音源
  fastify.get("/unblock/sources", (_, reply) => {
    return reply.send({
      sources: getSources(),
      custom: getCustomSources() != null,
      defaults: getDefaultSources(),
    });
  });
  // 设置解灰音源（空列表恢复默认）
  fastify.put("/unblock/sources", async (req, reply) => {
    const body = (req.body ?? {}) as { sources?: unknown };
    setCustomSources(Array.isArray(body.sources) ? body.sources : []);
    return reply.send({
      sources: getSources(),
      custom: getCustomSources() != null,
    });
